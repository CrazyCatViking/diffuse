...
export const buildRenderedDiffRowFields = (item: DisplayRow | undefined, options: {
  ...
  const oldReviewHighlights = diffRow && oldLine && oldText.length > 0 ? options.reviewHighlightsForLine('old', oldLine, oldText.length) : [];
  const newReviewHighlights = diffRow && newLine && newText.length > 0 ? options.reviewHighlightsForLine('new', newLine, newText.length) : [];
  ...
  return {
    ...
    inlineSyntaxSpans: diffRow?.kind === 'deleted'
      ? oldLine ? options.syntaxSpansForLine('old', oldLine) : undefined
      : newLine ? options.syntaxSpansForLine('new', newLine) : undefined,
